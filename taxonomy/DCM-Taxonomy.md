...
| **ITSM Action Policy** | The 9th DCM Policy output type (Placement Policy is the 8th). Side-effect policy that triggers an action in a connected ITSM system when a DCM event matches. Non-blocking by default (`on_failure: log_and_continue`). May gate the pipeline via `block_until_created: true` with mandatory timeout (ITSM-005). |
