...
- **Promotion-candidate detection — demand-driven; the engine *proposes*, never auto-promotes** (governance decides). It watches the authored corpus and surfaces what should become canonical:
  - **Missing Base/Type classes → promote-to-canonical.** When an org authors a Base or Type class because the canonical library **lacks** one — a gap-filler under its own authority (`acme.example/…`) — the engine flags it a **promote-to-canonical** candidate. A Base is meant to be shared, so a private fork of a category the library is missing is a promotion in waiting (UDLM ADR-038 *Naming depth*; the "author for promotion" best-practice).
  - **Recurring Provider-Class elements → suggest a new canonical `SharedDataElement`.** When the *same* element (by `(scope, name, schema)`) appears across **≥2 Provider classes** — multiple providers independently carrying it — the engine **suggests promoting it upward** to a Type/Base-scope canonical element (the ≥2-adopter signal; UDLM ADR-038 §6 *gated upward contribution*). The recurring-provider-element set is the observable roadmap of what to canonicalize; a suggestion lands `proposed` and canonicalizes by governance.
