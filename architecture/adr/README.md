...
| [022](022-credential-api-selection.md) | Credential API Selection | Credentials are brokered like placement: providers declare a granular capability+attestation matrix, requests state requirements, DCM selects the spec; value flows direct (CPX-001). Security/trust/fit > portability |
| [023](023-trust-model.md) | DCM Trust Model | Uphold · participate · expose a full trust model (PKI/mTLS, OAuth/OIDC, attestation) across 5 planes; DCM is a trust **broker**, not a credential authority; DCM-operational vs managed credential custody; market-graded by profile |
