...
| [024](024-rule-id-naming-and-registry.md) | Rule-ID Naming & Registry | Adopt UDLM ADR-028's PREFIX-NNN convention + central registry + registry-backed CI check (adopt-by-reference, ADR-021); the Capabilities Matrix is an index not a definition surface (collapses ~most of 68 apparent collisions); baseline-then-ratchet; DCM never re-mints UDLM IDs |
